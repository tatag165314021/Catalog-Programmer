...
if (isset($_POST['submit'])) {
    if (($_POST['password'] == $_POST['konfirm_password']) and email_validation($_POST['email'])) {
                ...
                if (mysqli_num_rows($mysql_get_users) > 0) {
                    ...
                    try {
                        ...
                        if ($number >= 1) {
                            ...
                            if ($jum_data > 0) {
                                $jum_data++;
                                $kode = str_pad($jum_data, 0, "0", STR_PAD_LEFT);
                                $user_id = "MU-" . $kode;
                            } else {
                                $user_id = "MU-1";
                            }
                            ...
                            $query1 = mysqli_query($con, "INSERT INTO user(user_id, email, nama, telepon, status, role, password, foto) VALUES ('$user_id','$email','$nama','$telepon','$status','$role','$pass_enkripsi','$image')");
                            for ($i = 0; $i < $number; $i++) {
            ...
            if (checkValue($_POST["skill"])) {
                ...
                if (mysqli_num_rows($mysql_get_users) > 0) {
                    ...
                    try {
                        ...
                        if ($number >= 1) {
                            $pass_enkripsi = encrypted($_POST['password']); 
                            $query1 = mysqli_query($con, "INSERT INTO user(user_id, email, nama, telepon, status, role, password) VALUES ('$user_id', '$email','$nama','$telepon','$status','$role','$pass_enkripsi')");
                            for ($i = 0; $i < $number; $i++) {
                                if (trim($_POST["skill"][$i] != '')) {
                                    $id_skill = base_convert(date('Ymd').microtime(false),10,36);
                                    $sql = "INSERT INTO skills(skill,bintang,user_id, id) VALUES('" . mysqli_real_escape_string($con, $_POST["skill"][$i]) . "'," . mysqli_real_escape_string($con, $_POST["bintang"][$i]) . ",'$user_id','$id_skill')";
        ...
        echo '<center>GAGAL, TOLONG ISI ULANG</center>';
    }
}
?>
<html>
                        ...
                        <!--<label style="margin-left:100px">Edit Profil Anda</label>-->
                            <center><h2>Register</h2>
                                ...
                                <div class="form-group">
                                    <div class="input-group">
                                        <span class="input-group-addon"><i class="fa fa-user"> </i></span>
                                        <input type="text" name="no_telp" placeholder="No HP atau Telepon" required="required" class="form-control">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <div class="input-group">
